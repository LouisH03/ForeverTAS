...
public:
    ...
    void MutateSparse(SparseMutationTimeline &timeline,
        ...
        std::mt19937 random = ModifierRandom(
                ...
                request.passIndex);
        const std::int64_t tick = request.tickDurationMs;
        const SparseMutationTimeline::Snapshot original =
        ...
        for (std::uint32_t index = 0u; index < steeringCount; ++index) {
            ...
            const std::int64_t end = std::min(
                    settings_.window.maximumTimeMs,
                    start + randomHold(settings_.steering.maximumHoldMs));
            const AnalogInputState previous = timeline.SteeringStateAt(start);
            const AnalogInputState value = settings_.steeringOffset
                    ? SaturateAnalogInputState(
                              static_cast<std::int64_t>(previous) +
                              RandomInteger<AnalogInputState>(
                    ...
                    : RandomInteger<AnalogInputState>(
                              ...
                              settings_.steeringAbsoluteMinimum,
                              settings_.steeringAbsoluteMaximum);
            timeline.EraseActionRange(
                    SandboxInputAction::Steer, start, end);
            timeline.AppendEvent(AnalogEvent(
                    start, SandboxInputAction::Steer, value));
            if (end > start) {
                timeline.AppendEvent(AnalogEvent(
                        end,
                        SandboxInputAction::Steer,
                        timeline.SteeringStateAt(original, end)));
            }
        }
        ...
        const auto insertSwitch = [&](const ChannelSettings &channel,
            ...
            for (std::uint32_t index = 0u; index < count; ++index) {
                ...
                const std::int64_t end = std::min(
                        settings_.window.maximumTimeMs,
                        start + randomHold(channel.maximumHoldMs));
                const bool previous = timeline.SwitchStateAt(action, start);
                timeline.EraseActionRange(action, start, end);
                timeline.AppendEvent(SwitchEvent(start, action, !previous));
                if (end > start) {
                    timeline.AppendEvent(SwitchEvent(
                            end,
                            action,
                            timeline.SwitchStateAt(original, action, end)));
                }
            }
        };
        insertSwitch(settings_.accelerate,
