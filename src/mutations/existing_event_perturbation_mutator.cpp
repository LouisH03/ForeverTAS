...
public:
    ...
    void MutateSparse(SparseMutationTimeline &timeline,
        ...
        if (settings_.toggleAccelerate) {
            eligibility = eligibility | MutationEligibility::Accelerate;
        }
        if (settings_.toggleBrake) {
            eligibility = eligibility | MutationEligibility::Brake;
        }
        ...
        const std::uint32_t requested = RandomInteger(
                random, settings_.minimumCount, settings_.maximumCount);
        const std::size_t count = std::min<std::size_t>(
                requested, eligible.size());
        const std::int64_t tick = request.tickDurationMs;
        const std::int64_t maximumShiftTicks = tick == 0
                ? 0
                : settings_.maximumTimeShiftMs / tick;
        for (std::size_t index = 0u; index < count; ++index) {
            const SparseMutationTimeline::EventHandle &handle =
                    eligible[index];
